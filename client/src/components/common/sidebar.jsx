...
export default function Sidebar({ isSideMenu }) {
  ...
  return (
    <Link to={to} className="block">
      <li className={` relative flex items-center py-2 px-3 my-1 font-medium rounded-md cursor-pointer transition-colors group ${active ? 'font-semibold bg-base-300' : 'font-normal text-gray-500'}  hover:bg-base-200 `}>
        ...
        {alert && (
          <div className={`absolute right-2 w-2 h-2 rounded  ${expanded ? '' : 'top-2'}`}></div>
        )}

        {!expanded && (
          <div className={`absolute left-full rounded-md px-2 py-1 ml-6 text-sm invisible opacity-0 transition-all shadow-md group-hover:visible group-hover:opacity-100  z-50 bg-white`}>
            {text}
          </div>
        )}
      </li>
