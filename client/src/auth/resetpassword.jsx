...
function Resetpassword() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const { success, errorMessage } = await useAuthStore.getState().resetPassword(token, formData.password, formData.confirmPassword);
          
            if (success) {
                setSuccessMessage('Password reset successfully');
                setErrorMessage('');
            } else {
                setErrorMessage(errorMessage || 'Failed to reset password');
                setSuccessMessage('');
            }
        } catch (error) {
                        ...
                        <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                            ...
                            <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
                                ...
                                <button
                                    ...
                                    className={`w-full text-white btn btn-primary focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center ${!passwordValid && 'cursor-not-allowed opacity-50'}`}
                                >
                                    Reset Password
                                </button>
